...
from bert_related import finetune_bert, load_bert, bert_eval
from cohesive_indices import read_cohesive_indices, concatenate_bert, with_TFIDF
import sys
...
# model_bert = finetune(train_texts, train_labels, "finetuned_BERT_for_newsela.sav")

# # When loading from model:
# model_bert = load_bert("finetuned_BERT_for_newsela.pt")
# true_labels, pred_labels = test_bert(test_texts, test_labels, model_bert)
...
coh_tfidf_scores = svmResults(randomSeed, coh_tfidf_features_train, coh_train_labels, coh_test_labels, coh_tfidf_features_test, 'SVM with Cohesive Indices')
...
ft_features = fasttext_features(4, data.target, refined_dat)


# print('*'*80)
# print('---------------BERT With Cohesive Indices---------------------------')

# true_labels, pred_labels = train_bert(test_texts, test_labels, model)
# bert_and_cohesive_features = concatenate_bert(true_labels, pred_labels, cohind)

# coh_train_texts, coh_test_texts, coh_train_labels, coh_test_labels = train_test_split(bert_and_cohesive_features, cohlabels, test_size=0.4)

# coh_BERT_scores = svmResults(randomSeed, coh_train_texts, coh_train_labels, coh_test_labels, coh_test_texts, 'SVM with Cohesive Indices')
...
data = load_files(p2, encoding='utf-8', decode_error='ignore')


print('*'*80)
print('---------------Starting with BERT---------------------------')
# true_labels, pred_labels = test_bert(data.data, data.target, model_bert)
...
train_models(4 ,fig_name, test_labels=data.target, test_data=refined_dat, epoch=20)
