...
def fasttext_features(ngram, test_labels, test_data, epoch=20):
    model = fasttext.train_supervised('cooking.train',lr=1,loss = 'ns',ws=70,wordNgrams=ngram,epoch=epoch)
    predicted_res = []

    for i in range(len(test_data)):
        prediction = model.predict(test_data[i], k=3)
        predicted_res.append([int(prediction[0][0][-1]), prediction[1][0], int(prediction[0][1][-1]), prediction[1][1]])
